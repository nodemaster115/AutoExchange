...
define([
    ...
    Graph.prototype.drawGameData = function () {
        ...
        if (Engine.gameState === 'ENDED') {
            ...
            var strCrashValue = '';
            if(Engine.tableHistory.length != 0)
                strCrashValue = Clib.formatDecimals(Engine.tableHistory[0].game_crash / 100, 2);
            ...
            var strNote = '';
            if(Engine.tableHistory.length != 0)
                strNote = languageFlag ? ('Busted @ ' + strCrashValue + 'x') : (' 爆点 @ ' + strCrashValue + 'x');
