            ...
            var rows = self.state.engine.tableHistory.slice(0, MAX_GAMES_SHOWED).map(function (game, i) {
                ...
                if (player) {
                    ...
                    if (game.game_crash == 0) {
                        ...
                        rangeBet = '-';
                    } else if(rangeBet.amount > 0) { // rangeBet
                        if(game.game_crash >= rangeBet.range_from && game.game_crash <=rangeBet.range_to)     //rangeBet success
                            profit = rangeBet.amount * rangeBet.range_multiplier;
                        else profit = profit - rangeBet.amount;

                        rangeBet = Clib.formatSatoshis(rangeBet.amount);
                        cashed_at = '-';
                        bet = '-';
                        extraBet = '-';
                        ...
                        if (player.stopped_at) {
                            ...
                            cashed_at = 'Busted';
                            profit = (-bet - extraBet- rangeBet.amount);
                        }
                ...
                var className;
                if (game.game_crash >= 198) { className = 'games-log-goodcrash'; } else if (game.game_crash <= 197) { className = 'games-log-badcrash'; } else { className = ''; }

                var styleLine = {};
                if (i % 2) {
                    styleLine = {color: '#ddd'};
                } else {
                    styleLine = {color: '#ddd', backgroundColor: '#313131'};
                }
                ...
                if (Engine.show_hash == 'show_hash') {
                    tbody_tr = D.tr({key: 'game_' + i, style: styleLine, className: 'class_gameLogTableTr'},
                        D.td(null,
                        ...
                        D.td(null,
                            D.div({ className: 'hash-copy-cont',
                                    ...
                                    // onClick: copyHash(game.game_id, game.hash),
            ...
            if (Engine.show_hash == 'show_hash') {
                thead = D.thead(null,
                    D.tr({style: {'color': '#efefef'}},
                        D.th(null, languageFlag ? 'CRASH' : '停止在'),
                        D.th(null, '@'),
                        ...
                        D.th(null, languageFlag ? 'HASH' : '哈希')
                    )
                );
            } else {
                thead = D.thead(null,
