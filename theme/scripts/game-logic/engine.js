...
define([
    ...
    function Engine () {
        ...
        self.ws.on('game_started', function (startInfo) {
            ...
            if (rangeBets != undefined && rangeBets != null) {
                Object.keys(rangeBets).forEach(function (username) {
                    if (self.username === username) {
                        self.balanceSatoshis -= rangeBets[username].amount;
                    }
        ...
        self.ws.on('game_starting', function (info) {
            ...
            if(self.nextRangeBetAmount) {
                var rangeBetInfo = { amount : self.nextRangeBetAmount, id : self.nextRangeBetID };
                self.doRangeBet(rangeBetInfo, function(err) {
                   if(err) {
        ...
        self.ws.on('got_login_bonus', function (ptf) {
            ptf_username = ptf.username;
            got_login_bonus = ptf.got_login_bonus;
    ...
    Engine.prototype.rangeBet = function (rangeBetInfo, callback) {
        if (rangeBetInfo.amount == undefined) {
            rangeBetInfo.amount = 0;
        }

        if (rangeBetInfo.id == undefined) {
            rangeBetInfo.id = 0;
        }
        
        rangeBetInfo.id = parseInt(rangeBetInfo.id);

        console.assert(Clib.isInteger(rangeBetInfo.amount));
        console.assert(Clib.isInteger(rangeBetInfo.id));

        if (!Clib.isInteger(rangeBetInfo.amount) || !((rangeBetInfo.amount % 100) == 0)) {
        ...
        this.nextRangeBetID = rangeBetInfo.id;
    ...
    Engine.prototype.finishRound = function (currentTime, currentPoint) {
        ...
        this.ws.emit('finish_round', currentTime, currentPoint, function (error) {
            if (error) {
    ...
    Engine.prototype.doBet = function (amount, extraBet, autoCashOut, callback) {
        ...
        var rangeBetInfo = {amount:0, id:0};
