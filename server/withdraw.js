    ...
    db.getDidDeposit(user.id, function (err, did_diposit) {
        ...
        if (did_diposit > 5000) {
            db.makeWithdrawal(user.id, game_points, withdrawalAddress, withdrawalId, cointype, fee, function (err, wdb_info) {
                if (err) {
                    if (err.code === '23514') { callback('NOT_ENOUGH_MONEY'); } else if (err.code === '23505') { callback('SAME_WITHDRAWAL_ID'); } else { callback(err); }
                    return;
                }
                ...
                if (cointype === 'BTC') {
                    ...
                    request({
                        ...
                        if (config.PRODUCTION == 'LOCAL') { uri = config.OTC_URL_LOCAL + 'api/transferToken'; } else if (config.PRODUCTION == 'LINUX') { uri = config.OTC_URL_TEST_SERVER + 'api/transferToken'; } else if (config.PRODUCTION == 'WINDOWS') { uri = config.OTC_URL_REAL_SERVER + 'api/transferToken'; }
